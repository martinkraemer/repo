jQuery.sap.declare("KapselSampleOrders.Component");
jQuery.sap.require("KapselSampleOrders.MyRouter");

sap.ui.core.UIComponent.extend("KapselSampleOrders.Component", {
	metadata : {
		name : "TDG Demo App",
		version : "1.0",
		includes : [],
		...
		rootView : "KapselSampleOrders.view.App",

		config : {
			resourceBundle : "i18n/messageBundle.properties",
			serviceConfig : {
				name: "/destinations/NorthwindService/Northwind/Northwind.svc/",
				serviceUrl: "/Northwind/Northwind.svc/Northwind/Northwind.svc/"
			}
		},

		routing : {
			config : {
				routerClass : KapselSampleOrders.MyRouter,
				viewType : "XML",
				viewPath : "KapselSampleOrders.view",
	...
	init : function() {
		...
		var oRootPath = jQuery.sap.getModulePath("KapselSampleOrders");
		...
		if(window.cordova && appContext && !window.sap_webide_companion && !devapp.externalURL) {
            ...
            if ((window.sap_webide_companion || devapp.externalURL) && device) {
				...
				var isSafari = false,  av = navigator.appVersion;
